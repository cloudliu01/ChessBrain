...
ACTION_SPACE_PLANES = 73  # Directional move encoding planes per AlphaZero
ACTION_SPACE_SIZE = ACTION_SPACE_PLANES * 8 * 8
BOARD_CHANNELS = 20  # 12 piece planes + side-to-move + castling + halfmove + extras per flowchart


class PolicyValueOutput(NamedTuple):
    policy_logits: TensorLike  # Shape: (batch, ACTION_SPACE_PLANES, 8, 8)
    value: TensorLike  # Shape: (batch, 1)

    def flatten_policy(self) -> TensorLike:
        """Flatten policy logits into (batch, ACTION_SPACE_SIZE)."""
    ...
    def log_probs(self, legal_mask: Optional[TensorLike] = None) -> TensorLike:
        """Return log probabilities with optional multiplicative legal mask."""
        if not TORCH_AVAILABLE:
            raise RuntimeError("PyTorch is required to compute log probabilities.")
        logits = self.flatten_policy()
        if legal_mask is not None:
            logits = logits + torch.log(legal_mask.clamp(min=1e-8))  # type: ignore[operator]
...
if TORCH_AVAILABLE:
    ...
    class ResidualBlock(nn.Module):
        """Single AlphaZero-style residual block."""

        def __init__(self, config: ResidualBlockConfig) -> None:
            super().__init__()
            padding = config.kernel_size // 2
            self.conv1 = nn.Conv2d(
                config.channels, config.channels, config.kernel_size, padding=padding, bias=False
            )
    ...
    class ValueHead(nn.Module):
        """Head approximating scalar value in range [-1, 1]."""
    ...
    class AlphaZeroResidualNetwork(nn.Module):
        ...
        def __init__(
            ...
            self.residual_blocks = nn.ModuleList(ResidualBlock(block_config) for _ in range(residual_blocks))
